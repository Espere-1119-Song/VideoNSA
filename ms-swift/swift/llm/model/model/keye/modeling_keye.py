...
class KeyeAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)


        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            attn_output = attn_output + causal_mask

        # Fix precision issues in float16 inference
        # Replace inf values with zeros in attention weights to prevent NaN propagation
        if query_states.dtype == torch.float16:
            attn_output = torch.where(torch.isinf(attn_output), torch.zeros_like(attn_output), attn_output)

        # upcast attention to fp32
        attn_output = nn.functional.softmax(attn_output, dim=-1, dtype=torch.float32).to(query_states.dtype)
...
class KeyeNSAVisionAttention(KeyeAttention):
    """
    NSA attention with learnable per-token, per-head gates.
    """
    ...
    def __init__(self, 
        ...
        **kwargs):
        ...
        # Gate网络修改为两层MLP with Sigmoid activation
        self.g_proj = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size // 4, bias=False),
            nn.Sigmoid(),
            nn.Linear(self.hidden_size // 4, self.num_heads * 3, bias=False)
        )
    ...
    def forward(
        ...
        if output_attentions:
            logger.warning_once(
                "KeyeModel is using KeyeNSAVisionAttention, but NSA does not support `output_attentions=True`. Falling back to the manual attention implementation."
            )
            return super().forward(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
        ...
        # Fix gate shapes: (B, M, num_heads, 3) -> unbind(-1) -> (B, M, num_heads)
        # g_proj已经包含Sigmoid激活，无需再次应用
        gates = self.g_proj(hidden_states).view(bsz, q_len, self.num_heads, 3)
        g_cmp, g_slc, g_swa = gates.unbind(-1)
        ...
        if past_key_value is not None:
            ...
            if cache_has_content:
                key_states, value_states = k_cached, v_cached
                key_states = rearrange(key_states, '... (h d) -> ... h d', d=self.head_dim)
                value_states = rearrange(value_states, '... (h d) -> ... h d', d=self.head_dim)

        # Expand query_states from [:, :, 32, :] to [:, :, 128, :] by repeating
        # Original shape: [bsz, q_len, num_heads, head_dim]
        # Target shape: [bsz, q_len, 128, head_dim]
        original_num_heads = query_states.shape[2]  # 32
        target_num_heads = 128
        repeat_factor = target_num_heads // original_num_heads  # 128 // 32 = 4
        
        # Repeat query_states along the head dimension
        query_states_expanded = query_states.unsqueeze(3).expand(-1, -1, -1, repeat_factor, -1).contiguous()
        query_states_expanded = query_states_expanded.view(query_states.shape[0], query_states.shape[1], target_num_heads, query_states.shape[3])
        
        # Also expand g_cmp, g_slc, g_swa to match the expanded query_states
        # Original shapes: [bsz, q_len, 32]
        # Target shapes: [bsz, q_len, 128]
        g_cmp_expanded = g_cmp.unsqueeze(2).expand(-1, -1, repeat_factor, -1).contiguous()
        g_cmp_expanded = g_cmp_expanded.view(g_cmp.shape[0], g_cmp.shape[1], target_num_heads)
        
        g_slc_expanded = g_slc.unsqueeze(2).expand(-1, -1, repeat_factor, -1).contiguous()
        g_slc_expanded = g_slc_expanded.view(g_slc.shape[0], g_slc.shape[1], target_num_heads)
        
        g_swa_expanded = g_swa.unsqueeze(2).expand(-1, -1, repeat_factor, -1).contiguous()
        g_swa_expanded = g_swa_expanded.view(g_swa.shape[0], g_swa.shape[1], target_num_heads)
        

        from nsa.nsa import nsa_func
        attn_output = nsa_func(
            query_states_expanded, key_states, value_states,
            g_cmp=g_cmp_expanded,
            g_slc=g_slc_expanded,
            g_swa=g_swa_expanded,
        ...
        # Adjust the output to maintain the same final result
        # The expanded query_states will produce output with more heads, so we need to average corresponding positions
        # Reshape attn_output to group the repeated heads
        attn_output = attn_output.view(bsz, q_len, original_num_heads, repeat_factor, -1)
        # Average corresponding positions (e.g., positions 0, 32, 64, 96 for the first head)
        attn_output = attn_output.mean(dim=3)  # Average across repeat_factor dimension (positions 0, 32, 64, 96)
        # Reshape back to the expected format
        attn_output = attn_output.reshape(bsz, q_len, -1).contiguous()
...
QWEN3_ATTENTION_CLASSES = {
    "eager": KeyeAttention,
    "flash_attention_2": KeyeNSAVisionAttention,
    "sdpa": KeyeNSAVisionAttention
}
