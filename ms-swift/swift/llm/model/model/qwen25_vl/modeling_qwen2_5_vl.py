...
class Qwen2_5_VLMixNSA(Qwen2_5_VLAttention):
    ...
    def __init__(self, 
        ...
        **kwargs):
        ...
        self.g_proj_2.weight.requires_grad = True
        self.g_proj_2.bias.requires_grad = True
        
    
    def _reset_gate_initialization(self):
        ...
        target_prob = 0.5
        logit_val = torch.log(torch.tensor(target_prob) / (1 - target_prob))  # ≈ -0.847
    ...
    def _mixed_attention(self, query_states, key_states, value_states, is_vision_tokens, hidden_states, attention_mask, q_len):
        ...
        vision_mask_expanded = is_vision_tokens.unsqueeze(1).unsqueeze(3).expand_as(query_states)
        text_mask_expanded = ~vision_mask_expanded
        
        zeros = torch.zeros_like(query_states)

        
        vision_only_q = torch.where(vision_mask_expanded, query_states, zeros)
        kv_mask_expanded = is_vision_tokens.unsqueeze(1).unsqueeze(3).expand_as(key_states)
        vision_only_k = torch.where(kv_mask_expanded, key_states, torch.zeros_like(key_states))
        vision_only_v = torch.where(kv_mask_expanded, value_states, torch.zeros_like(value_states))
        ...
        g_cmp_64 = torch.cat([g_cmp, g_cmp, torch.zeros(bsz, q_len, 8, device=g_cmp.device, dtype=g_cmp.dtype)], dim=-1)
        g_slc_64 = torch.cat([g_slc, g_slc, torch.zeros(bsz, q_len, 8, device=g_slc.device, dtype=g_slc.dtype)], dim=-1)
        g_swa_64 = torch.cat([g_swa, g_swa, torch.zeros(bsz, q_len, 8, device=g_swa.device, dtype=g_swa.dtype)], dim=-1)
        
        query_states_padded = torch.cat([
            vision_only_q, vision_only_q, 
            torch.zeros(bsz, 8, q_len, self.head_dim, device=vision_only_q.device, dtype=vision_only_q.dtype)
        ], dim=1).transpose(1, 2)
        

        from nsa.nsa import nsa_func
        nsa_output_64 = nsa_func(
            query_states_padded, vision_only_k.transpose(1, 2), vision_only_v.transpose(1, 2),
            g_cmp=g_cmp_64, g_slc=g_slc_64, g_swa=g_swa_64,
            block_count=self.block_counts, block_size=self.block_size,
            window_size=self.window_size, scale=None, layer_idx=self.layer_idx
        )
        
        front_28 = nsa_output_64[:, :, :28, :]
        copied_28 = nsa_output_64[:, :, 28:56, :]
        nsa_output = ((front_28 + copied_28) * 0.5).transpose(1, 2)
        
        text_only_q = torch.where(text_mask_expanded, query_states, zeros)
        ...
        text_only_q = text_only_q.transpose(1, 2)  # [bsz, q_len, num_heads, head_dim]
        full_k = full_k.transpose(1, 2)            # [bsz, q_len, num_heads, head_dim]  
        full_v = full_v.transpose(1, 2)            # [bsz, q_len, num_heads, head_dim]
        ...
        sdpa_output = flash_output.transpose(1, 2)  # [bsz, num_heads, q_len, head_dim]

        attn_output = nsa_output + sdpa_output
