    ...
    if load_model:
        ...
        if model is None:
            if model_info.task_type == 'seq_cls' and not model_meta.is_reward:
                ...
                context = partial(patch_automodel, **context_kwargs)
            with context():
                if 'Keye' in model_dir:
                    from .model.keye.modeling_keye import KeyeForConditionalGeneration
                    model = KeyeForConditionalGeneration.from_pretrained(
                        model_dir, config=model_config, torch_dtype=torch_dtype, trust_remote_code=True, **model_kwargs)
                elif 'Qwen' in model_dir or 'checkpoint' in model_dir or 'doneright' in model_dir:
                    from .model.qwen25_vl.modeling_qwen2_5_vl import DonerightForConditionalGeneration
                    # from transformers import Qwen2_5_VLForConditionalGeneration
                    model = DonerightForConditionalGeneration.from_pretrained(
                        model_dir, config=model_config, torch_dtype=torch_dtype, trust_remote_code=True, **model_kwargs)
                else:
...
def get_model_tokenizer_with_flash_attn(model_dir: str,
    ...
    if model_config is None:
        if 'Keye' in model_dir:
            from .model.keye.configuration_keye import KeyeConfig
            model_config = KeyeConfig.from_pretrained(model_dir, trust_remote_code=True)
        else:
            model_config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True)
...
def get_model_tokenizer_multimodal(model_dir: str, *args, **kwargs):
    ...
    if 'Keye' in model_dir:
        from .model.keye.processing_keye import KeyeProcessor
        processor = KeyeProcessor.from_pretrained(model_dir, trust_remote_code=True)
    else:
